...
uses
  ...
  KM_Houses, KM_HouseWoodcutters, KM_Hand, KM_HouseWatchTower,
...
const
  BLOCKED_BY_PEACETIME: set of TKMGameInputCommandType = [gicArmySplit, gicArmySplitSingle,
    ...
    gicArmyFormation,  gicArmyWalk, gicArmyStorm, gicHouseBarracksEquip, gicHouseTownHallEquip, gicHouseSiegeWorkshopTrain];
...
uses
  ...
  KM_HandsCollection,
  KM_HouseMarket, KM_HouseBarracks, KM_HouseSchool,KM_HouseSiegeWorkshop, KM_HouseTownHall, KM_HouseStore, KM_HouseArmorWorkshop,
...
begin
  ...
  begin
    ...
    if CommandType in [gicHouseRepairToggle, gicHouseDeliveryModeNext, gicHouseDeliveryModePrev, gicHouseWoodcuttersCutting, gicHouseTownHallMaxGold, gicHouseTowerMaxStone,
      gicHouseOrderProduct, gicHouseMarketFrom, gicHouseMarketTo, gicHouseBarracksRally, gicHouseSiegeWorkshopRally ,gicHouseTownHallRally,
    ...
    case CommandType of
      ...
      gicHouseTowerMaxStone:   TKMHouseTower(srcHouse).StoneMaxCnt := EnsureRange(IntParams[1], 0, High(Word));
...
begin
  Assert(aCommandType in [gicHouseRemoveTrain, gicHouseSchoolTrainChLastUOrder, gicHouseTownHallMaxGold, gicHouseRemoveTrainSiege, gicHouseSiegeWorkshopTrainChLastUOrder, gicHouseTowerMaxStone]);
  Assert((aHouse is TKMHouseSchool) or (aHouse is TKMHouseTownHall) or (aHouse is TKMHouseSiegeWorkshop) or (aHouse is TKMHouseTower));  //zmiana
...
begin
  Assert((aCommandType = gicHouseBarracksRally) or (aCommandType = gicHouseSiegeWorkshopRally) or (aCommandType = gicHouseTownHallRally) or (aCommandType = gicHouseWoodcuttersCutting));
  Assert((aHouse is TKMHouseBarracks) or (aHouse is TKMHouseSiegeWorkshop) or (aHouse is TKMHouseTownHall) or (aHouse is TKMHouseWoodcutters));
