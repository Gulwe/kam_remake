...
procedure TKMHouseSiegeWorkshop.DemolishHouse(aFrom: TKMHandID; IsSilent: Boolean = False);
var
  I: Integer;
begin
...
function TKMHouseSiegeWorkshop.AddUnitToQueue(aUnit: TKMUnitType; aCount: Byte): Byte;
var
  I, K: Integer;
begin
  Result := 0;
...
begin
  ...
  begin //Make sure unit started training
    ...
    if DeliveryMode = dmTakeOut then
      gHands[Owner].Deliveries.Queue.AddOffer(Self, wtWood, 4);
      gHands[Owner].Deliveries.Queue.AddOffer(Self, wtSteel, 4);
  end;
  fUnitWip := nil;
...
function TKMHouseSiegeWorkshop.GetResInLocked(aI: Byte): Word;
begin
...
procedure TKMHouseSiegeWorkshop.ChangeUnitTrainOrder(aNewPosition: Integer);
begin
...
//Change unit priority in training queue
procedure TKMHouseSiegeWorkshop.ChangeUnitTrainOrder(aOldPosition, aNewPosition: Integer);
var
  tmpUnit: TKMUnitType;
  I: Byte;
begin
...
begin
  ...
  if DeliveryMode = dmTakeOut then
    gHands[Owner].Deliveries.Queue.RemOffer(Self, wtWood, 4);
  if DeliveryMode = dmTakeOut then
    gHands[Owner].Deliveries.Queue.RemOffer(Self, wtSteel, 4);
...
begin
  ...
  if (CheckResIn(wtWood) <> 5) or (CheckResIn(wtSteel) <> 5)  then exit; //There must be enough gold to perform training
...
procedure TKMHouseSiegeWorkshop.UnitTrainingComplete(aUnit: Pointer);
begin
  ...
  if (CheckResIn(wtWood) = 5 )and (CheckResIn(wtSteel) = 5) then
  begin
    ResTakeFromIn(wtWood, 5); //Do the goldtaking
    ResTakeFromIn(wtSteel, 5); //Do the goldtaking
    gHands[Owner].Stats.WareConsumed(wtWood);
    gHands[Owner].Stats.WareConsumed(wtSteel);
...
begin
  if fUnitWip = nil then
    ...
    Result := (
              Byte(haWork1 in CurrentAction.SubAction) * 1 +
