...
const
  ...
  DISTRIBUTION_DEFAULTS: array [1..5, 1..4] of Byte = (
    (5, 5, 5, 0),
    (5, 3, 4, 4),
    (3, 4, 5, 0),
    (4, 5, 3, 0),
...
type
  ...
  private
    fWareDistribution: array [1..5, 1..5] of Byte;
...
begin
  case aWare of
    wtSteel: if aHouse = htWeaponSmithy   then fWareDistribution[1,1] := aValue else
             if aHouse = htSiegeWorkshop  then fWareDistribution[1,2] := aValue else
              if aHouse = htArmorSmithy    then fWareDistribution[1,3] := aValue;
    ...
    wtWood:  if aHouse = htArmorWorkshop  then fWareDistribution[3,1] := aValue else
              if aHouse = htSiegeWorkshop  then fWareDistribution[3,2] := aValue else
              if aHouse = htWeaponWorkshop then fWareDistribution[3,3] := aValue;
    ...
    wtTrunk: if aHouse = htCharcoalFactory then fWareDistribution[5,1] := aValue else
             if aHouse = htSawmill        then fWareDistribution[5,2] := aValue;
...
begin
  ...
  case aWare of
    wtSteel: if aHouse = htWeaponSmithy   then Result := fWareDistribution[1,1] else
            if aHouse = htSiegeWorkshop  then Result := fWareDistribution[1,2]  else
              if aHouse = htArmorSmithy    then Result := fWareDistribution[1,3];
    ...
    wtWood:  if aHouse = htArmorWorkshop  then Result := fWareDistribution[3,1] else
             if aHouse = htSiegeWorkshop  then Result := fWareDistribution[3,2] else
              if aHouse = htWeaponWorkshop then Result := fWareDistribution[3,3];
    wtCorn:  if aHouse = htMill           then Result := fWareDistribution[4,1] else
     ...
     wtTrunk: if aHouse = htCharcoalFactory then Result := fWareDistribution[5,1] else
             if aHouse = htSawmill        then Result := fWareDistribution[5,2];
