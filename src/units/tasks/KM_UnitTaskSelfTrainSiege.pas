unit KM_UnitTaskSelfTrainSiege;
{$I KaM_Remake.inc}
interface
uses
  Classes, SysUtils,
  KM_CommonClasses, KM_Defaults, KM_Houses, KM_HouseSchool,KM_HouseSiegeWorkshop, KM_Units;


//Train citizen in school
type
  TKMTaskSelfTrainSiege = class(TKMUnitTask)
  private
    fSchool: TKMHouseSiegeWorkshop;
  public
    constructor Create(aUnit: TKMUnit; aSchool: TKMHouseSiegeWorkshop);
...
{ TKMTaskSelfTrainSiege }
constructor TKMTaskSelfTrainSiege.Create(aUnit: TKMUnit; aSchool: TKMHouseSiegeWorkshop);
begin
  inherited Create(aUnit);

  fType := uttSelfTrainSiege;
  fSchool   := TKMHouseSiegeWorkshop(aSchool.GetPointer);
  fUnit.Visible := False;
end;


constructor TKMTaskSelfTrainSiege.Load(LoadStream: TKMemoryStream);
begin
  inherited;
  LoadStream.CheckMarker('TaskSelfTrainSiege');
  LoadStream.Read(fSchool, 4);
end;


procedure TKMTaskSelfTrainSiege.SyncLoad;
begin
  inherited;
  fSchool := TKMHouseSiegeWorkshop(gHands.GetHouseByUID(Cardinal(fSchool)));
end;


destructor TKMTaskSelfTrainSiege.Destroy;
begin
...
function TKMTaskSelfTrainSiege.Execute:TKMTaskResult;
begin
...
procedure TKMTaskSelfTrainSiege.Save(SaveStream: TKMemoryStream);
begin
  inherited;
  SaveStream.PlaceMarker('TaskSelfTrainSiege');
